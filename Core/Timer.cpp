...
uint64 Timer::getElapsedMilliSeconds() const {
	return (_pause ? _pauseTime - _beginTime : GetMicroSeconds() - _beginTime) / 1000U;
}

uint64 Timer::getElapsedSeconds() const {
	return (_pause ? _pauseTime - _beginTime : GetMicroSeconds() - _beginTime) / 1000000U;
}
...
uint64 Timer::getElapsedPauseMilliSeconds() const {
	return _pause ? 0 : (GetMicroSeconds() - _pauseTime) / 1000U;
}

uint64 Timer::getElapsedPauseSeconds() const {
	return _pause ? 0 : (GetMicroSeconds() - _pauseTime) / 1000000U;
}

uint64 Timer::GetMicroSeconds() {
	struct timeval tp;
	int ans = 0;
	if ((ans = gettimeofday(&tp, NULL)) != 0) {
		printf("gettimeofday::%d\n", gettimeofday);
	}
	return tp.tv_sec * 1000000U + tp.tv_usec;
}

uint64 Timer::GetMilliSeconds() {
	struct timeval tp;
	gettimeofday(&tp, NULL);
	return tp.tv_sec * 1000U + tp.tv_usec / 1000U;
}

uint64 Timer::GetSeconds() {
	struct timeval tp;
	gettimeofday(&tp, NULL);
	return tp.tv_sec + tp.tv_usec / 1000000U;
}
